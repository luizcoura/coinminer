...
    .then(function (response) {
    ...
    if(isNaN(mmep1[mmep1.length-1])) {
      ...
      var m = 2 / (p1 + 1);
      mme = (+response.data[limit - 2][4] - mmep1[mmep1.length-1]) * m + mmep1[mmep1.length-1];
      mmep1.push(mme);
    }
    ...
    for (let i = 0; i < limit - 1; i++) {
      var a = Number.parseFloat(mmep1[i]).toFixed(6);
